...
{
    /// <summary>
    /// UI drop slot for equipment items.
    /// Step 4 scope: visual feedback + drag/drop handler surface only.
    /// </summary>
    [RequireComponent(typeof(CanvasGroup))]
    public sealed class EquipmentDropSlotView : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        [SerializeField, Tooltip("Tint applied while an equipment icon is equipped. Defaults to white to avoid disabled-looking icons.")]
        ...
        [SerializeField, Tooltip("If enabled, logs equipment slot drag/drop diagnostics.")]
        ...
        public IEquipmentService EquipmentService => _equipmentService;

        // This static is intentionally simple for Step 4.
        public static bool IsDragging { get; set; }
        public static bool IsDraggingEquippedItem { get; private set; }
        public static EquipmentSlotType? DraggingFromSlot { get; private set; }
        public static string DraggingDefinitionId { get; private set; }
        public static EquipmentDefinition DraggingEquipmentDefinition { get; private set; }
        public static bool AnyDragActive => IsDragging || IsDraggingEquippedItem || InventoryItemDragHandler.IsDraggingItem || UnitDragHandler.IsDragging;

        public event Action<EquipmentDropSlotView, PointerEventData> DropReceived;
        ...
        private EquipmentDefinition _equippedDefinition;
        ...
        private static EquipmentDropSlotView _activeDragHandler;
        ...
        private static void ResetStaticState()
        {
            ...
            DraggingEquipmentDefinition = null;
        ...
        public void SetEquippedItem(string definitionId, EquipmentDefinition definition)
        {
        ...
        {
            // Step 4 stub. Wired in later steps.
        }
        ...
        public void OnDrop(PointerEventData eventData)
        {
            ...
            EquipmentDefinition draggedDefinition = InventoryItemDragHandler.DraggingEquipmentDef;
            if (CanEquipDraggedDefinition(draggedDefinition) &&
                _equipmentService != null &&
                _selectedUnit != null &&
                _equipmentService.TryEquip(_selectedUnit, draggedDefinition))
            {
                SetEquippedItem(draggedDefinition.Id, draggedDefinition);
        ...
        {
            if (InventoryItemDragHandler.IsDraggingItem || UnitDragHandler.IsDragging)
            {
                if (_enableDiagnostics)
                {
                    Core.Diagnostics.SBLog.Info(
                        $"EquipmentDropSlotView[{_slotType}]: OnBeginDrag ignored (inventoryDragging={InventoryItemDragHandler.IsDraggingItem}, unitDragging={UnitDragHandler.IsDragging}).",
                        this);
                }
                return;
            }
            ...
            if (_activeDragHandler != null && _activeDragHandler != this)
            {
                ...
                {
                    Core.Diagnostics.SBLog.Info(
                        $"EquipmentDropSlotView[{_slotType}]: OnBeginDrag ignored because another equipment slot drag is active.",
                        this);
                }
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(_equippedDefinitionId))
            {
                if (_enableDiagnostics)
                {
                    Core.Diagnostics.SBLog.Info($"EquipmentDropSlotView[{_slotType}]: OnBeginDrag ignored because slot has no equipped definition.", this);
                }
                return;
            }
                ...
                if (_ghostImage != null)
                {
                    ...
                    if (_enableDiagnostics && _ghostImage.sprite == null)
                    {
                        Core.Diagnostics.SBLog.Warn(
                            $"EquipmentDropSlotView[{_slotType}]: Drag ghost sprite is null for definition '{_equippedDefinitionId ?? "<null>"}'.",
                            this);
                    }
                }
            }

            IsDraggingEquippedItem = true;
            DraggingFromSlot = _slotType;
            DraggingDefinitionId = _equippedDefinitionId;
            DraggingEquipmentDefinition = _equippedDefinition;
            ...
            {
                Core.Diagnostics.SBLog.Info(
                    $"EquipmentDropSlotView[{_slotType}]: Drag started for definition '{_equippedDefinitionId}' (ghost={(_dragGhostRoot != null ? _dragGhostRoot.name : "<null>")}).",
                    this);
            }
        }
        ...
        private void OnDisable()
        {
            bool thisOwnsActiveDrag =
                _isDraggingThis ||
                (IsDraggingEquippedItem &&
                 DraggingFromSlot.HasValue &&
                 DraggingFromSlot.Value == _slotType &&
        ...
        private void EnsureHighlightOverlay()
        {
            ...
            {
                var go = new GameObject("EquipmentDropHighlight", typeof(RectTransform), typeof(CanvasRenderer), typeof(Image), typeof(CanvasGroup));
        ...
        {
            bool canEquipDragged = CanEquipDraggedDefinition(InventoryItemDragHandler.DraggingEquipmentDef);
            if (!canEquipDragged)
            {
                _pulseTime = 0f;
        ...
        private bool CanEquipDraggedDefinition(EquipmentDefinition draggedDefinition)
        {
            if (draggedDefinition == null || draggedDefinition.SlotType != _slotType)
            {
                return false;
            }

            if (_equipmentService == null || _selectedUnit == null)
            {
                return false;
            }

            return _equipmentService.CanEquip(_selectedUnit, draggedDefinition);
        }
        ...
        private void EnsureDragGhostRoot()
        {
            if (_dragGhostRoot != null)
            {
                ...
                {
                    Core.Diagnostics.SBLog.Warn(
                        $"EquipmentDropSlotView[{_slotType}]: Assigned drag ghost root '{_dragGhostRoot.name}' is under an inactive parent chain. Re-resolving ghost root.",
                        this);
                }
            ...
            {
                Core.Diagnostics.SBLog.Info(
                    $"EquipmentDropSlotView[{_slotType}]: Created runtime drag ghost root '{_dragGhostRoot.name}' under root canvas '{rootCanvas.name}'.",
                    this);
            }
        }
        ...
        private void SetGhostScreenPosition(PointerEventData eventData)
        {
            ...
            {
                Core.Diagnostics.SBLog.Warn(
                    $"EquipmentDropSlotView[{_slotType}]: Failed to convert ghost screen position. Falling back to world position.",
                    this);
            }
        }
